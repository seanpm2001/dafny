...
module {:extern "Microsoft.Dafny.Compilers.SelfHosting.CSharp"} CSharpDafnyCompiler {
  ...
  module Translator {
    ...
    function method TranslateBinary(b: C.BinaryExpr) : D.Expr
      ...
      reads *
    {
      var op, e0, e1 := b.CSharp__theResolvedOp, b.E0, b.E1;
    ...
    function method TranslateLiteral(l: C.LiteralExpr): D.Expr reads * {
      if l.Value is Boolean then
        ...
        D.LiteralExpr(D.LitReal(DafnyUtils.TypeConv.AsReal(l.Value))) // TODO test
  ...
  method WriteAST(wr: DafnyUtils.SyntaxTreeAdapter, ast: Target.StrTree) {
    match ast {
      case Str(s) =>
        wr.Write(s);
  ...
  class {:extern} DafnyCSharpCompiler {
    ...
    method Compile(dafnyProgram: CSharpDafnyAST.Program,
      ...
      var st := new DafnyUtils.SyntaxTreeAdapter(wr);
